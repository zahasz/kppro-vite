...
<div x-data="{
    open: false,
    get isDark() {
        return typeof $store !== 'undefined' && $store.theme 
            ? $store.theme.isDark 
            : (typeof window.ThemeStore !== 'undefined' ? window.ThemeStore.isDark : false);
    },
    get userChosen() {
        return typeof $store !== 'undefined' && $store.theme 
            ? $store.theme.userChosen 
            : (typeof window.ThemeStore !== 'undefined' ? window.ThemeStore.userChosen : false);
    },
    toggleTheme() {
        if (typeof $store !== 'undefined' && $store.theme) {
            $store.theme.toggle();
        } else if (typeof window.ThemeStore !== 'undefined') {
            window.ThemeStore.toggle();
        }
    },
    resetPreference() {
        if (typeof $store !== 'undefined' && $store.theme) {
            $store.theme.resetToSystemPreference();
        } else if (typeof window.ThemeStore !== 'undefined') {
            window.ThemeStore.resetToSystemPreference();
        }
    }
}" class="relative">
    ...
    @if ($variant === 'dropdown')
        ...
        <div 
            ...
            x-cloak
        >
            <button @click="toggleTheme(); open = false" class="{{ $class }}" :class="{'bg-gray-100 dark:bg-gray-700': !isDark}">
                <span>Tryb jasny</span>
                <svg x-show="!isDark" class="h-5 w-5 ml-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            ...
            <button @click="resetPreference(); open = false" class="{{ $class }}" :class="{'bg-gray-100 dark:bg-gray-700': !userChosen}">
                <span>Preferencje systemowe</span>
                <svg x-show="!userChosen" class="h-5 w-5 ml-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                </svg>
            </button>
        </div>
    @else
        ...
        >
            ...
            @if ($variant === 'button')
                <span class="ml-2" x-text="isDark ? 'Tryb jasny' : 'Tryb ciemny'"></span>
            @endif
        </button>
    @endif
</div> 