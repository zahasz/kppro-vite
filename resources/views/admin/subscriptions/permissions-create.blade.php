<x-admin-layout>
    <x-slot name="header">
        Dodaj nowe uprawnienie subskrypcji
    </x-slot>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="mb-5">
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            Dodaj nowe uprawnienie i określ w jakich planach subskrypcji ma ono być dostępne.
                        </p>
                    </div>

                    <form method="POST" action="{{ route('admin.subscriptions.permissions.store') }}">
                        @csrf

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nazwa uprawnienia</label>
                                <input type="text" name="name" id="name" value="{{ old('name') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Nazwa wyświetlana dla użytkowników">
                                @error('name')
                            ...
                            <div>
                                ...
                                <input type="text" name="code" id="code" value="{{ old('code') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Unikalny identyfikator uprawnienia, np. module.feature">
                                @error('code')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Krótki opis tego uprawnienia">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <select id="category" name="category" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                ...
                                <option value="finance" {{ old('category') == 'finance' ? 'selected' : '' }}>Finanse</option>
                                <option value="warehouse" {{ old('category') == 'warehouse' ? 'selected' : '' }}>Magazyn</option>
                                <option value="documents" {{ old('category') == 'documents' ? 'selected' : '' }}>Dokumenty</option>
                                <option value="integrations" {{ old('category') == 'integrations' ? 'selected' : '' }}>Integracje</option>
                                <option value="reports" {{ old('category') == 'reports' ? 'selected' : '' }}>Raporty</option>
                                <option value="users" {{ old('category') == 'users' ? 'selected' : '' }}>Użytkownicy</option>
                                <option value="other" {{ old('category') == 'other' ? 'selected' : '' }}>Inne</option>
                            </select>
                            @error('category')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="feature_flag" id="feature_flag" value="{{ old('feature_flag') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Identyfikator w systemie funkcji flagowych (jeśli używany)">
